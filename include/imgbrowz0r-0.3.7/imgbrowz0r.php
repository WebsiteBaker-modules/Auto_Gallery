		...
		foreach ($this->files as $k => $file)
		{
			if ($file[0] === 1)
			{
				...
				echo "\t\t", '<div class="img-thumbnail img-column-', $t_row_count, '"><a href="', $this->config['images_url'],
					 ...
					 $image_thumbnail, '\')" rel ="prettyPhoto[gallery]" title="', $file[1], '">', $file[1], '</a><span>', $this->format_time($file[3]),
				     '</span></div>', "\n";
			}
			else
			{
...
if (file_exists($this->full_path.$file[1].'/.desc')){
			echo '<br />'.strip_tags(file_get_contents($this->full_path.$file[1].'/.desc'), '<p><strong><em><a><br><h1><h2><h3>');
			echo '</div>';
}
echo "</div>";				
	...
	/*added function for Auto_Gallery*/
	public function current_dir()
		{ $current_dir =substr($this->full_path,0,-1);
			return "$current_dir";
			}
